...
export function Schedules() {
  ...
  return (
    <div className="space-y-5">
      ...
      {message && <div className={`rounded-xl px-4 py-3 text-sm font-semibold ${message.includes('Không thể') || message.includes('đã có') ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-800'}`}>{message}</div>}
      ...
      <div className="card flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="grid gap-2 text-sm sm:grid-cols-3">
          ...
          <div className="rounded-xl bg-green-50 p-3 text-green-800"><b>{schedules.filter((s) => s.dayOfWeek === todayDayOfWeek).length}</b><p>Ca hôm nay</p></div>
        </div>
      </div>
      ...
      {loading ? <div className="card">Đang tải lịch tập...</div> : (
        <div className="grid gap-4 xl:grid-cols-7">
          {grouped.map((day) => (
            <section key={day.value} className={`rounded-3xl border bg-white p-4 shadow-sm ${day.value === todayDayOfWeek ? 'border-green-300 ring-2 ring-green-100' : 'border-slate-100'}`}>
