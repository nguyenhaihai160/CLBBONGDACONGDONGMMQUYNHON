...
export function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[2rem] bg-slate-950 p-5 text-white shadow-2xl shadow-slate-950/15 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="mb-2 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-lime-200">Instagram-like Operations</p>
          ...
          <div className="grid grid-cols-2 gap-3 sm:flex">
            <Link to="/students" className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-slate-950 shadow-lg">+ Học viên</Link>
            <Link to="/attendance" className="rounded-2xl bg-lime-300 px-4 py-3 text-center text-sm font-black text-slate-950 shadow-lg">Điểm danh</Link>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.4fr_0.9fr]">
        <div className="card rounded-[2rem]">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-green-700">Activity feed</p>
          ...
          <div className="space-y-3">
            {feed.map((item) => (
              <Link key={item.title} to={item.to} className="flex gap-3 rounded-3xl border border-slate-100 bg-slate-50 p-3 transition hover:border-green-200 hover:bg-green-50">
                ...
                <span>
                  <b className="block text-slate-950">{item.title}</b>
        ...
        <div className="space-y-4">
          ...
          <div className="card rounded-[2rem] bg-gradient-to-br from-green-700 to-slate-950 text-white">
            <CalendarCheck className="mb-4" />
            <h2 className="text-xl font-black">Tác vụ Admin</h2>
            <p className="mt-2 text-sm text-green-50">Tạo lớp, gán HLV, xác nhận học phí, quản lý QR, lịch tập, kho đồ, chatbot và Zalo OA.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        ...
        <QuickCard to="/users" icon={ShieldCheck} title="Phân quyền" desc="Tạo/xóa HLV, khóa tài khoản." />
...
function StoryBubble({ to, icon: Icon, label, value }: { to: string; icon: any; label: string; value: any }) {
  return (
    <Link to={to} className="flex min-w-[92px] flex-col items-center gap-2 rounded-[1.75rem] bg-white p-3 text-center shadow-sm ring-1 ring-slate-100 transition hover:-translate-y-0.5 hover:shadow-md">
      <span className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-lime-300 via-green-500 to-slate-950 p-[3px]">
...
function QuickCard({ to, icon: Icon, title, desc }: { to: string; icon: any; title: string; desc: string }) {
  return (
    <Link to={to} className="card group rounded-[2rem] transition hover:-translate-y-1 hover:border-green-200 hover:shadow-xl">
      <Icon className="mb-3 text-green-700 transition group-hover:scale-110" />
