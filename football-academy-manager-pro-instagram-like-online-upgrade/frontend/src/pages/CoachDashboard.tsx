...
export function CoachDashboard() {
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-slate-950 via-green-900 to-green-600 p-5 text-white shadow-2xl shadow-green-950/20 sm:p-6">
        <p className="mb-2 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-lime-100">Coach mobile mode</p>
        <h1 className="text-3xl font-black sm:text-4xl">Sân tập hôm nay</h1>
        <p className="mt-2 max-w-2xl text-sm text-green-50 sm:text-base">HLV mở điện thoại ngoài sân, xem lớp mình, lịch tập và điểm danh nhanh như dùng app.</p>
        <div className="mt-5 grid grid-cols-2 gap-3 sm:flex">
          <Link to="/attendance" className="rounded-2xl bg-lime-300 px-4 py-3 text-center text-sm font-black text-slate-950 shadow-lg">Điểm danh ngay</Link>
          <Link to="/coach-classes" className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-slate-950 shadow-lg">Lớp của tôi</Link>
        </div>
      </section>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="card rounded-[2rem]">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-green-700">Today schedule</p>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {data.todaySchedules.map((s: any) => (
              <Link to="/attendance" key={s.id} className="rounded-3xl border border-slate-100 bg-slate-50 p-4 transition hover:border-green-200 hover:bg-green-50">
                <p className="font-black text-slate-950">{s.class.name}</p>
...
function Action({ to, title, desc }: { to: string; title: string; desc: string }) {
  return (
    <Link to={to} className="block rounded-3xl border border-slate-100 bg-slate-50 p-3 transition hover:border-green-200 hover:bg-green-50">
