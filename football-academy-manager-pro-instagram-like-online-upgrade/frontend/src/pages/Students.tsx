...
export function Students() {
  ...
  function openEditForm(student: StudentItem) {
    ...
    setForm({
      ...
      monthlyFeeAmount: Number(student.monthlyFeeAmount || 0),
    });
    setMessage('');
    setShowForm(true);
  ...
  async function submit(e: FormEvent) {
    ...
    const payload = {
      ...
      monthlyFeeAmount: Number(form.monthlyFeeAmount) > 0 ? Number(form.monthlyFeeAmount) : null,
    };
      ...
      {showForm && (
        <form onSubmit={submit} className="card space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label className="space-y-1">
              <span className="text-sm font-bold text-slate-700">Học phí riêng / tháng</span>
              <input className="input" type="number" min={0} value={form.monthlyFeeAmount} onChange={e => setForm({ ...form, monthlyFeeAmount: Number(e.target.value) })} placeholder="0 = lấy theo lớp" />
            </label>
            <label className="space-y-1">
        ...
        {filteredStudents.map((s) => (
          <div key={s.id} className="card grid gap-3 md:grid-cols-[1.6fr_1fr_1fr_1fr_auto] md:items-center">
            <div className="flex items-center gap-3">
              {s.avatarUrl ? <img src={s.avatarUrl} alt={s.fullName} className="h-12 w-12 rounded-2xl object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-green-100 font-black text-green-800">{s.fullName.charAt(0)}</div>}
              <div>
            ...
            <div>
              ...
              <p className="font-semibold">{s.class?.name || 'Chưa phân lớp'}</p>
              {s.monthlyFeeAmount ? <p className="text-xs font-bold text-green-700">Phí riêng: {Number(s.monthlyFeeAmount).toLocaleString('vi-VN')}đ/tháng</p> : <p className="text-xs text-slate-400">Theo phí lớp</p>}
            </div>
