    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-[380px_1fr]">
        <div className="space-y-3">
          {classes.map(c => (
            <button key={c.id} onClick={() => setSelectedClassId(c.id)} className={`w-full rounded-2xl border p-4 text-left shadow-sm ${selectedClassId === c.id ? 'border-green-500 bg-green-50' : 'border-slate-100 bg-white hover:bg-slate-50'}`}>
...
function Stat({ title, value, icon }: { title: string; value: number; icon: ReactNode }) {
  return <div className="card flex items-center gap-3"><div className="rounded-xl bg-green-50 p-3 text-pitch">{icon}</div><div><p className="text-sm text-slate-500">{title}</p><b className="text-xl">{value}</b></div></div>;
}
...
function AttendanceBadge({ status }: { status?: string }) {
  if (status === 'PRESENT') return <span className="rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-700">Có mặt</span>;
  if (status === 'ABSENT') return <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-bold text-red-700">Vắng</span>;
  if (status === 'EXCUSED') return <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700">Xin phép</span>;
