      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
        <form onSubmit={saveConnection} className="card space-y-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${connection.connected ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>{connection.connected ? 'Đã cấu hình' : 'Chưa cấu hình'}</span>
          </div>
        ...
        <div className="space-y-2">
          {messages.map((m) => (
            <div key={m.id} className="rounded-2xl bg-slate-50 p-3 text-sm">
              <div className="flex flex-col justify-between gap-1 sm:flex-row"><b>{m.parentPhone}</b><span className="text-xs font-bold text-green-700">{m.status}</span></div>
