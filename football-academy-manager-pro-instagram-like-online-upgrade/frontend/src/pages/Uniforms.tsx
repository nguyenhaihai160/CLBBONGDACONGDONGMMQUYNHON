                ...
                <tr key={p.id} className="border-t">
                  ...
                  {user?.role === 'ADMIN' && (
                    <td className="space-x-2">
                      ...
                      {p.isActive ? <button className="rounded-xl bg-red-50 px-3 py-2 font-semibold text-red-700" onClick={() => deleteProduct(p)}><Trash2 className="inline" size={15} /> Xóa</button> : <button className="rounded-xl bg-green-50 px-3 py-2 font-semibold text-green-700" onClick={() => restoreProduct(p)}><ArchiveRestore className="inline" size={15} /> Khôi phục</button>}
                    </td>
                  )}
                </tr>
              ))}
...
function InventoryBadge({ status }: { status: string }) {
  const cls = status === 'Hết hàng' ? 'bg-red-100 text-red-700' : status === 'Sắp hết' ? 'bg-amber-100 text-amber-700' : status === 'Đã xóa/ẩn' ? 'bg-slate-100 text-slate-500' : 'bg-green-100 text-green-700';
