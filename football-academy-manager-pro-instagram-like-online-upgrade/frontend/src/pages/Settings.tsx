...
export function Settings() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setMessage('Đã lưu cấu hình CLB, logo và QR học phí. Tải lại trang nếu muốn cập nhật logo ở toàn bộ giao diện.');
    } catch (error: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_380px]">
        <form onSubmit={submit} className="card space-y-6">
          ...
          <section className="space-y-4">
            <h2 className="font-black">QR nộp học phí trên thẻ học viên</h2>
            <div className="grid gap-4 md:grid-cols-2">
              <label className="space-y-1"><span className="text-sm font-bold text-slate-700">Ngân hàng</span><input className="input" value={settings.bankName} onChange={(e) => update('bankName', e.target.value)} placeholder="VD: ACB, Vietcombank..." /></label>
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <span className="text-xs text-slate-500">QR này sẽ hiển thị trên thẻ học viên và trang in thẻ.</span>
              </div>
            </div>
          </section>
        ...
        <div className="card h-fit space-y-4">
          <p className="text-sm font-bold uppercase tracking-widest text-green-700">Xem trước thẻ</p>
          <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-green-700 to-slate-950 p-5 text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-100">{settings.academyShortName || 'Tên rút gọn'}</p>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center gap-3 rounded-2xl bg-white p-3 text-slate-900">
              ...
              <div className="text-sm">
                <b>QR nộp học phí</b>
                <p>{settings.bankName || 'Ngân hàng'}</p>
                <p>{settings.bankAccountNumber || 'Số tài khoản'}</p>
                <p>{settings.bankAccountName || 'Tên chủ tài khoản'}</p>
              </div>
            </div>
          </div>
          <p className="text-xs text-slate-500">Nội dung chuyển khoản gợi ý trên từng thẻ: {settings.paymentNotePrefix || 'HP'} + mã học viên.</p>
        </div>
      </div>
    </div>
  );
