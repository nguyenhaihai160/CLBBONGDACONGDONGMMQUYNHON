...
function getLearningStatus(student: any) {
  ...
  return { label: 'ĐANG HỌC', className: 'bg-green-100 text-green-800 border-green-200', note: `Còn ${remaining}/${total} buổi` };
}
...
function StudentCard({ student, settings }: { student: any; settings: Settings }) {
  ...
  return (
    <article className="student-print-card overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
      <div className="bg-gradient-to-br from-green-700 via-emerald-600 to-slate-950 p-4 text-white">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate text-xs uppercase tracking-widest text-green-100">{settings.academyShortName || 'Football Academy'}</p>
      ...
      <div className="p-4">
        <div className="flex gap-4">
          {student.avatarUrl ? <img src={student.avatarUrl} className="h-24 w-24 rounded-3xl object-cover" /> : <div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-green-100 text-4xl font-black text-green-800">{student.fullName.charAt(0)}</div>}
          <div className="min-w-0 flex-1">
            <p className="text-xs font-black uppercase tracking-widest text-green-700">{student.studentCode}</p>
        ...
        <div className="mt-4 flex items-center gap-3 rounded-3xl border border-slate-100 bg-slate-50 p-3">
          ...
          <div className="min-w-0 text-xs text-slate-600">
            ...
            <p className="mt-1 rounded-xl bg-white px-2 py-1 font-bold text-green-700">ND: {paymentContent}</p>
          </div>
        </div>
