...
export function Login() {
  ...
  async function checkServer() {
    ...
    try {
      ...
      // Debug chỉ là thông tin hỗ trợ. Ở production endpoint này có thể được tắt để an toàn.
      try {
        ...
        setDebugText('Debug DB đang tắt hoặc không khả dụng. Health API vẫn hoạt động.');
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top_left,#dcfce7,transparent_40%),linear-gradient(135deg,#052e16,#0f172a)] p-4">
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        <div className="mb-6 rounded-3xl bg-gradient-to-r from-green-700 to-lime-400 p-5 text-white">
          <p className="text-sm uppercase tracking-[0.3em] text-green-50">Academy</p>
          <h1 className="text-3xl font-black">Football Manager Pro</h1>
          <p className="mt-2 text-sm text-green-50">Bản single-port: web và API chạy chung một cổng 5173.</p>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="rounded-2xl bg-slate-50 p-3 text-xs text-slate-600">
            ...
            {debugText && <p className="mt-1 font-bold text-green-700">{debugText}</p>}
