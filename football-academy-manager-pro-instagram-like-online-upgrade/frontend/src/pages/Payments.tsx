...
function getStudentMonthlyFee(student: any) {
  return Number(student?.monthlyFeeAmount || student?.class?.feeAmount || 0);
}
...
export function Payments() {
  ...
  function handleStudentChange(studentId: string) {
    ...
    const nextAmount = form.feeType === 'MONTHLY' ? getStudentMonthlyFee(selected) : form.amount;
    setForm({ ...form, studentId, amount: nextAmount || form.amount });
  }

  function handleFeeTypeChange(feeType: string) {
    const selected = students.find(s => s.id === form.studentId);
    const nextAmount = feeType === 'MONTHLY' ? getStudentMonthlyFee(selected) : form.amount;
    setForm({ ...form, feeType, amount: nextAmount || form.amount });
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={submit} className="card grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        <select className="input xl:col-span-2" value={form.studentId} onChange={e => handleStudentChange(e.target.value)} required>
          <option value="">Chọn học viên</option>
          {students.map(s => <option key={s.id} value={s.id}>{s.studentCode} - {s.fullName}</option>)}
        </select>
        <select className="input" value={form.feeType} onChange={e => handleFeeTypeChange(e.target.value)}>
        ...
        <div className="space-y-1">
          ...
          <p className="px-1 text-[11px] font-semibold text-slate-500">Có thể sửa riêng từng học viên/tháng</p>
        </div>
